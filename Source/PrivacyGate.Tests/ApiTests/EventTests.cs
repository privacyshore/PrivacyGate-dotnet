...
namespace PrivacyGate.Tests.ApiTests
{
   public class EventTests : CommerceApiTests
   {
      ...
      public async Task can_list_events()
      {
         ...
         server.ShouldHaveCalled("https://api.privacygate.io/events")
      ...
      public async Task list_events_with_different_ordering()
      {
         ...
         server.ShouldHaveCalled("https://api.privacygate.io/events?order=asc&limit=75&starting_after=ffff&ending_before=gggg")
            .WithVerb(HttpMethod.Get);
      }
      ...
      public async Task can_get_a_single_event()
      {
         ...
         server.ShouldHaveCalled($"https://api.privacygate.io/events/{EventData.EventModel.Id}")
            .WithVerb(HttpMethod.Get);
      }
   }
}
